...
def registration(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id,
                         f'Очень приятно, {user_name}. Для того, чтоб получить и начать сдавать задачи напишите /contest')
    except Exception:
...
def name_edition(message):
    ...
    bot.send_message(message.chat.id,
                     f'Очень приятно, {new_nickname}. Для того, чтоб получить и начать сдавать задачи напишите /contest')
...
def check_ans(message, task_number):
    if task_number == 1:
        if message.text == ans1:
            ...
            cur.execute('UPDATE Competitors SET score = ROUND(score + ?*(1-?)) WHERE user_id = ?',
                        (score1, fine, user_id))
            connection.commit()
            connection.close()
            score1 = score1 * 0.95
            ...
            bot.send_message(message.chat.id,
                             f'Верно! Вы сдали задачу {number_of_solutions1}-м. Чтобы ответить на другие задачи, нажмите на их номер')
    ...
    if task_number == 2:
        if message.text == ans2:
            ...
            score2 = score2 * 0.95
            bot.send_message(message.chat.id,
                             f'Верно! Вы сдали задачу {number_of_solutions2}-м. Чтобы ответить на другие задачи, нажмите на их номер')
            bot.register_next_step_handler(message, handle_button)
        else:
            connection = sqlite3.connect('results.sql')
            cur = connection.cursor()
    ...
    if task_number == 3:
        if message.text == ans3:
            ...
            score3 = score3 * 0.95
            bot.send_message(message.chat.id,
                             f'Верно! Вы сдали задачу {number_of_solutions3}-м. Чтобы ответить на другие задачи, нажмите на их номер')
            bot.register_next_step_handler(message, handle_button)
        else:
            connection = sqlite3.connect('results.sql')
            cur = connection.cursor()
    ...
    if task_number == 4:
        if message.text == ans4:
            ...
            score4 = score4 * 0.95
            bot.send_message(message.chat.id,
                             f'Верно! Вы сдали задачу {number_of_solutions4}-м. Чтобы ответить на другие задачи, нажмите на их номер')
            bot.register_next_step_handler(message, handle_button)
        else:
            connection = sqlite3.connect('results.sql')
            cur = connection.cursor()
